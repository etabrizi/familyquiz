...
function Navigation({ navigation, currentSelection, changeSelection, isChecked, showAnswers }) {
    ...
    let navList = navigation.map(item => <li key={item.id}><a id={item.id} className={currentSelection === item.id ? "active" : ""} href="#" onClick={changeSelection}><span className={`icon-${item.icon ? item.icon : `flag`}`}></span>{item.linkCopy}</a></li>);
